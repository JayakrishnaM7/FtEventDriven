...
public class FtBillpayment {
	Logger logger = LoggerFactory.getLogger(FtBankTransferNonupiApi.class);
	@Autowired
	FtBillpaymentI ftBillpaymentI;
	@PostMapping(path = "/ftBillPayment")
	public ResponseEntity<?> ftBillpayment(@RequestBody FtRq ftRq) throws Exception {
		ResponseEntity<?> rs=ftBillpaymentI.ftBillpaymentIprocess(ftRq);
