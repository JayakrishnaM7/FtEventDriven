...
public class FtSelfTransfer {
	Logger logger = LoggerFactory.getLogger(FtBankTransferNonupiApi.class);
	@Autowired
	FtSelfTransferI ftSelfTransferI;
	
	@PostMapping(path = "/ftSelfTransfer")
	public ResponseEntity<?> ftSelfTransfer(@RequestBody FtRq ftRq) throws Exception {
		ResponseEntity<?> rs=ftSelfTransferI.ftSelfTransferprocess(ftRq);
	...
	@PostMapping(path = "/ftLoanTransfer")
	public ResponseEntity<?> ftLoanTransfer(@RequestBody FtRq ftRq) throws Exception {
		ResponseEntity<?> rs=ftSelfTransferI.ftSelfTransferprocess(ftRq);
		Account2accountRs account2accountRs=new Account2accountRs();
		Thread.sleep(2000);
		logger.info("rs.getStatusCode() :"+rs.getStatusCode());
	...
	@PostMapping(path = "/ftDepositAccountTransfer")
	public ResponseEntity<?> ftDepositAccount(@RequestBody FtRq ftRq) throws Exception {
		ResponseEntity<?> rs=ftSelfTransferI.ftSelfTransferprocess(ftRq);
		Account2accountRs account2accountRs=new Account2accountRs();
		Thread.sleep(2000);
		logger.info("rs.getStatusCode() :"+rs.getStatusCode());
	...
	public ResponseEntity<?> ftMurualfundDepositAccount(@RequestBody FtRq ftRq) throws Exception {
		ResponseEntity<?> rs=ftSelfTransferI.ftSelfTransferprocess(ftRq);
		Account2accountRs account2accountRs=new Account2accountRs();
		Thread.sleep(2000);
		logger.info("rs.getStatusCode() :"+rs.getStatusCode());
